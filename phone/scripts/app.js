...
$(document).ready(function () {
    ...
    ctxSip = {
        ...
        config: {
            ...
            transportOptions: {
                wsServers: ["wss://" + user.WSServer + ":4433"],
                traceSip: true
            },
            log: {
        ...
        getUserMediaFailure: function (e) {
            ...
            ctxSip.setError(true, 'Lỗi Micro.', 'Bạn phải cho phép truy cập micro. Kiểm tra trên thanh địa chỉ.', true);
        },
        ...
        sipCall: function (target) {
            ...
            try {
                var s = ctxSip.phone.invite(target + "@" + user.Realm, { inviteWithoutSdp: false, rel100: SIP.C.supported.SUPPORTED });
        ...
        sipTransfer: function (sessionid) {
            ...
            var s = ctxSip.Sessions[sessionid],
                target = window.prompt('Nhập số để chuyển', '');

            ctxSip.setCallSessionStatus('<i>Đang chuyển cuộc gọi...</i>');
            s.refer(target);
        },
        ...
        hasWebRTC: function () {
            ...
            if (navigator.webkitGetUserMedia) {
                ...
                ctxSip.setError(true, 'Trình duyệt không hỗ trợ.', 'Trình duyệt của bạn không hỗ trợ những yêu cầu thiết yếu.');
                window.console.error("WebRTC không hỗ trợ");
                return false;
            }
        }
    };
    ...
    ctxSip.phone.on('disconnected', function (e) {
        ...
        // disable phone
        ctxSip.setError(true, 'Websocket Disconnected.', 'Đã có lỗi xảy ra trong quá trình kết nối websocket.');
    ...
    ctxSip.phone.on('registered', function (e) {
        ...
        var closeEditorWarning = function () {
            return 'Bạn sẽ không thể gọi đi hoặc nhận cuộc gọi từ trình duyệt sau khi đóng cửa sổ này.';
        };
    ...
    ctxSip.phone.on('registrationFailed', function (e) {
        ctxSip.setError(true, 'Lỗi Đăng Ký.', 'Lỗi xảy ra trong quá trình đăng ký. Kiểm tra lại thông tin cài đặt.');
        ctxSip.setStatus("Lỗi: Đăng ký thất bại");
    });
